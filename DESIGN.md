...
`sm` was, for most of this codebase's history, the only breakpoint the layout used, and an earlier version of this file said so as a flat rule. That was a description of the codebase at a moment, not a law: the running-timer chip's Complete and Pause buttons are now `hidden lg:inline-flex` (`RunningTimerChip.tsx`), hidden below the `lg` breakpoint (1024px). They are the one pair of controls in the top bar that is genuinely redundant rather than merely large — both are duplicated on the item's own row in Planning (`ItemRow.tsx`'s Start/Complete actions), so hiding them at narrow widths costs a shortcut, not a capability the timer chip alone provides. `md` was tried first and rejected: it still showed the buttons at exactly 768px, where the bar was measured 59px too wide with them present, so `lg` is the breakpoint that actually clears the row. This is the second breakpoint the layout uses, not a precedent for a third: it exists because this specific pair of controls has a working duplicate elsewhere, and that test — is the hidden control's function available somewhere else on screen — is what a new breakpoint proposal has to pass, not "the bar feels crowded." The one other breakpoint in the codebase besides `sm` and this `lg` is `md:text-sm` on `Input` and `Textarea`, the stock 16px-on-mobile rule that stops iOS zooming a focused field; it is a field-level fix, not a layout tier, and new work should not read it as an invitation to add one either.
