...
- **Signal Red** (destructive `hsl(358.8 100% 69.6%)` dark / `hsl(357.1 100% 45.3%)` light; urgency-critical `hsl(358 75% 59%)` dark / `hsl(358 75% 46%)` light — distinct tunings of the same hue): Destructive actions and the "critical" urgency band. The `destructive` value has one further use that is neither: removed lines in the session pane's patch view, paired with Success Green on added lines — see Success Green's fourth examined use for why marking diff polarity there is not an urgency claim.
- **Success Green** (`hsl(142 71% 45%)` dark / `hsl(142 71% 35%)` light): Defined on the `Badge` component (`variant="success"`). Reserved for exactly one meaning: "this is saved or done and needs no action," never for a status that demands attention. It has four examined uses, each admitted against the Two-Band/color-scarcity rule below rather than by default:
  ...
  4. Added lines in the session pane's patch view (`SessionDiff.tsx`), paired with Signal Red on removed lines. This is the one use of either hue that is not a status at all: `+`/`-` mark which side of a diff a line is on, the same distinction a terminal's own colored diff output makes, and the component's own comment says so — "the one place in the app where those two hues are not a Two-Band Rule attention claim." It cannot compete with the urgency bands because it never appears where a reading does: the patch is behind its own disclosure, three sections away from any score chip or session-state glyph, and the file headers (`+++`/`---`) are excluded so the color marks content, not diff syntax. Green here means "added," nothing else — it carries no verdict on whether the addition is good.
...
**The scroll gap.** With a timer running, the bar's non-shrinking content — the two floored outer tracks, the mode switch, and the running-timer chip's own intrinsic width — used to come to roughly 860px, none of which the `minmax(0,20rem)` search track could absorb by shrinking further once it hit zero. Hiding the running-timer chip's Complete and Stop buttons below `lg` (see below) shrank that non-shrinking width, and closes part of the gap: measured with a timer running, the bar now overflows by 63px at 700px and 123px at 640px, down from a wider band before those buttons were hidden. From `sm` (640px), where the floor engages, up to roughly 760px, that arithmetic still doesn't close and the page scrolls horizontally instead of the bar reflowing — a narrower band than the roughly 640–860px it was before this phase, not a closed one. This is why the floor is gated on `sm` rather than left on everywhere: below 640px the floor is dropped so the logo compresses instead, which is the lesser evil only at the very narrowest widths and not a fix for the scroll range above it. Closing what remains needs a different narrow layout for the bar — two rows, or a drawer for the actions that don't fit — which is a redesign nobody has chosen, not a tweak that got skipped this phase. This is a known limitation, stated as one, not a resolved one.
...
`sm` was, for most of this codebase's history, the only breakpoint the layout used, and an earlier version of this file said so as a flat rule. That was a description of the codebase at a moment, not a law: the running-timer chip's Complete and Stop buttons are now `hidden lg:inline-flex` (`RunningTimerChip.tsx`), hidden below the `lg` breakpoint (1024px). This is not a free hide: Complete and Stop are duplicated on the item's own row, but that row lives in Planning, and the timer chip is global — it is the one place to act on the running timer from `/work`, `/report`, or `/settings`. Below `lg` on any route other than Planning, stopping or completing the running timer means navigating to Planning first; there is no shortcut for it anywhere else on screen. That was accepted anyway, because the alternative was worse: `md` was tried first and rejected because it still showed the buttons at exactly 768px, where the bar was measured 59px too wide with them present, and short of `lg` the only way to clear the row was a horizontally scrolling top bar on every route, all the time. This is the second breakpoint the layout uses, not a precedent for a third: a new breakpoint proposal has to name the capability it costs and the alternative it avoids as plainly as this paragraph does, not lean on "the bar feels crowded." The one other breakpoint in the codebase besides `sm` and this `lg` is `md:text-sm` on `Input` and `Textarea`, the stock 16px-on-mobile rule that stops iOS zooming a focused field; it is a field-level fix, not a layout tier, and new work should not read it as an invitation to add one either.
...
The house grammar for "there is more here, and here is exactly how much." Used in seven places — `Snoozed · 3` and a group's `Lower scoring · 4` in `SignalsBoard.tsx` (one file, two separate controls), `Parked · 2` in `ItemSection.tsx`, `Ended · N` in the Watch Floor rail (`SessionRosterRail.tsx`), `Didn't fit · N` and `Deferred by your lean · N` in the suggestion panel (one `renderExcluded()` function in `SuggestPanel.tsx`, called twice — one function, two controls), and `Patch · N` in the session pane's diff (`SessionDiff.tsx`) — and it must be used for any eighth. Counted with `grep -rl aria-expanded components/`: five files match, not seven, because two of them (`SignalsBoard.tsx`, `SuggestPanel.tsx`) each hold more than one control — a file count and a control count are not the same number here.
...
- **Label:** a state, then an interpunct, then the count in the mono/tabular register: `Parked · 2`. The label names *what* the hidden rows are, not the act of revealing them. "Show more" is not a state and is not an acceptable label.
...
- **Keyboard:** should carry `data-row-nav` so `j`/`k` row navigation stops on it (`GlobalKeymapProvider.tsx`) rather than stepping over the rows it hides. Enter and Space activate it natively. **`Lower scoring`, `Ended` (`SessionRosterRail.tsx`), both suggestion-panel controls (`Didn't fit` / `Deferred by your lean`, `SuggestPanel.tsx`), and `Patch` (`SessionDiff.tsx`) currently do.** `Snoozed` (`SignalsBoard.tsx`) and `Parked` (`ItemSection.tsx`) are reachable by Tab but invisible to `j`/`k`, so a keyboard user walking the list with `j` steps straight past them and never learns the rows exist. Treat the attribute as required on all seven; two of them are outstanding.
...
- **Planning shows it too, quieter.** `ItemRow.tsx` renders the identical glyph and word beside a row's title whenever that item has a live session, reading the same `agentStateDisplay()` table and the same `AGENT_STATE_GLYPHS`/`AGENT_TONE_CLASS` maps (`components/AgentStateGlyph.tsx`) the rail does, so the two surfaces can never disagree about what a state is called. It never animates, unlike the rail's `working` glyph (`motion-safe:animate-spin`, `SessionRosterRow.tsx`): the rail is a single-session detail view, but a Planning row is one of many on a surface read for its scores, and a spinner on every agent row would pull the eye off them (`ItemRow.tsx`'s own comment says so).
- **No gold.** Nothing in this vocabulary is ever Threadline Gold, however live the session — see The One Thread Rule.
- **The rail's Today mark stays muted too.** When a session's item is pinned to Today, the rail shows a `Pin` glyph and the word "Today" beside the state (`SessionRosterRow.tsx`), reusing the same icon Planning's own pin/unpin-from-today control uses (`ItemRow.tsx`) rather than inventing a second signal. It renders at the row's own muted tone, not Threadline Gold — even though the Today card's own left edge is gold (see Cards / Containers below). The two are not in conflict, because they mark different facts: the card's gold edge is The One Thread Rule's "your thread, live" signal, and the rail's mark is only saying the item sits on today's plan. A delegated session fails the *mine* clause regardless of which day its item is pinned to, so the mark that reads "on today's plan" cannot also read "your thread" — a gold-edged Today card sitting above a gray session row is the correct rendering of two true, separate facts, not a contradiction the rail failed to resolve.

### Session Pane
`SessionPane.tsx`, the detail view for the session selected in the Watch Floor rail: header state, then a fixed action row, then Diff and Transcript sections.

- **The action row never moves.** It sits directly under the header, and the header holds only the title and the state label, neither of which reflows. Everything the rest of the pane can show or hide — the explanation sentence, the needs-you question, the branch badge — renders below the action row, never above it or inside it, because a hook event landing mid-visit must not shift what is under the cursor: a click aimed at Dismiss must never land on a control that appeared a moment before (`SessionPane.tsx`). The branch badge is kept out of this row for the identical reason and lives beside the Diff heading instead, since `diff.available` can flip across polls just as the explanation text can.
- **The Warp readout is deliberately not a button.** Warp has no API to focus an existing tab, per the phase 1 spike into Warp (`SessionPane.tsx`'s own comment), and dismissing a session deletes its tab config precisely so the session cannot be reopened into a second agent process (`app/api/agent-sessions/[id]/dismiss/route.ts`). A "Reveal" button here would launch a second process, not reveal the first one. The pane instead renders the tab's colour swatch and title as plain text ("switch to Warp by hand to see it"), pinned to the row with `ml-auto` so its position never depends on whether Resume is rendered beside it.
- **The transcript's live region is not the transcript.** `SessionTranscript.tsx` renders the message list with `aria-live="off"`, stated in the component's own comment: the list is replaced wholesale on every poll, and a live region wired to a log that gets rewritten that often reads the whole page aloud continuously, which makes the transcript unusable with a screen reader rather than more accessible. `SessionPane.tsx` owns the one live region the pane has — a separate `sr-only` `role="status" aria-live="polite"` element — and it speaks state transitions only, gated on a ref comparison so a poll that changes nothing about the state does not re-announce a sentence already read.
- **The diff shows the branch, not the agent.** `SessionDiff.tsx`, reading `lib/agent-worktree.ts`'s `readSessionDiff()`: everything on the session's branch that is not on the default branch, framed on screen as "On `<branch>`, compared with `<base>`." This is deliberately not "what the agent changed," and `readSessionDiff()`'s own doc comment says no code path may call it that: Ariadne records no base commit at the moment a session launches, so it cannot separate the agent's commits from commits already on the branch before or alongside it. A branch Chris had been working on himself shows his own work here too. The label is load-bearing, not a cosmetic choice of words — it is the one framing that stays true regardless of who made which commit.
- **The patch is the disclosure control's seventh place.** `Patch · N` (`SessionDiff.tsx`) joins the six pre-existing consumers of the pattern — see Disclosure Control. Added lines render in Success Green and removed lines in Signal Red, excluding the `+++`/`---` file headers so only content lines carry the colour; see Success Green's fourth examined use for why marking diff polarity with those two hues is not a Two-Band Rule urgency claim.
...
**Focus is inset.** `ring-1 ring-ring ring-inset` on the segment, not the offset ring the buttons use. A segment has neighbours on both sides and a track edge 2px away, so an offset ring would either collide with the next segment or spill over the frame; drawn inside the segment's own box it lands cleanly whichever notch has focus. This is one of five focus treatments in the system, three of them inside the same 64px top bar: `ring-1 ring-offset-2` for buttons (`ui/button.tsx`), offset because a neutral hairline drawn on top of the filled Primary variant disappears into it; `ring-1` drawn flush for fields (`ui/input.tsx`, `ui/textarea.tsx`), which carry no fill for a ring to vanish into and so need no offset; `ring-2 ring-offset-1` for the score chip, doubled and pulled tight against its 20px target; `ring-1 ring-inset` here, responding to the neighbours on both sides; and `ring-2 ring-offset-2` in Threadline Gold for the logo link and the search trigger (`TopBar.tsx`), the only one that answers to meaning rather than to size — a button's offset, doubled and in gold, because these two controls are marked as "about the product's identity" and nothing else earns that hue. Each is a response to the target's size, surroundings, or meaning, not a free choice.
